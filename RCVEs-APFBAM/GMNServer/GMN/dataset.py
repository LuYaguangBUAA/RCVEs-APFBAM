...
class GraphEditDistanceDataset_OneHot(GraphSimilarityDataset):
    """Graph edit distance dataset.  One_Hot"""
    ...
    """
        ...
        one_hot_inds = np.random.randint(low=0, high=self._nclass, size=n_total_nodes)
        #one_hot_inds_number = np.random.randint(low=0, high=self._nnumber, size=n_total_nodes)
        one_hot_inds_edge = np.random.randint(low=0, high=4, size=n_total_edges)
        one_hot_inds_angle = np.random.randint(low=0, high=9, size=n_total_nodes)
        node_features_label=np.eye(self._nclass, dtype=np.float32)[one_hot_inds]
        #node_features_number=np.eye(self._nnumber, dtype=np.float32)[one_hot_inds_number]
        generator = cycle_one_hot(self._nnumber, n_total_nodes)
        node_features_number = np.array([next(generator) for _ in range(n_total_nodes)])
        node_features_angle = np.eye(9, dtype=np.float32)[one_hot_inds_angle]
        
        one_hot_indices_kind = [np.random.choice(self._nnumber, self._nnumber, replace=False) for _ in range((int)(n_total_nodes/self._nnumber))]
        
        one_hot_subarrays_kind = [create_one_hot_subarray(indices) for indices in one_hot_indices_kind] 
        one_hot_array_kind = np.vstack(one_hot_subarrays_kind)  
        
        angles = np.random.uniform(0, 360, n_total_nodes)
        angles_radians = np.radians(angles)  
        sin_values = np.sin(angles_radians, dtype=np.float32)  
        cos_values = np.cos(angles_radians, dtype=np.float32)  
        angle_direction_features = np.column_stack((sin_values, cos_values)) 
        ...
        return GraphData(
            ...
            # with high dimensional features.
            # node_features=np.ones((n_total_nodes, 8), dtype=np.float32),
            #node_features=np.concatenate((node_features_label, one_hot_array_kind, angle_direction_features), axis=1),
            #node_features=np.concatenate((node_features_label, angle_direction_features), axis=1),
            node_features=one_hot_array_kind,
            # edge_features=np.ones((n_total_edges, 4), dtype=np.float32),
...
def create_one_hot_subarray(indices):  
    '''
    生成不重复的onehot
    '''
    one_hot = np.zeros((14, 14), dtype=np.float32)  

    one_hot[np.arange(14), indices] = 1  
...
def cycle_one_hot(num_classes, num_samples):
    '''
    循环生成one-hot
    '''
    index = 0
    while True:
        # 创建一个长度为 num_classes 的零向量
        one_hot = np.zeros(num_classes, dtype=np.float32)
        # 设置对应的位置为 1
        one_hot[index % num_classes] = 1
        yield one_hot
...
class FixedGraphEditDistanceDataset_OneHot(GraphEditDistanceDataset_OneHot):
    """A fixed dataset of pairs or triplets for the graph edit distance task.
  This dataset can be used for evaluation.
  """
    ...
    def __init__(
        ...
        super(FixedGraphEditDistanceDataset_OneHot, self).__init__(
            n_nodes_range,
            p_edge_range,
            n_changes_positive,
            n_changes_negative,
