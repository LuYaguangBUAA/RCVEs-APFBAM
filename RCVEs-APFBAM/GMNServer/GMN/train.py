...
# print("node_feature_dim", node_feature_dim, config['encoder']['node_feature_dim'])
...
training_n_graphs_in_batch_size = config['training']['batch_size']
...
#n_graphs = torch.tensor([training_n_graphs_in_batch])
...
for i_iter in range(config['training']['n_training_steps']):
    ...
    if config['training']['mode'] == 'pair':
        ...
        #print('ss', node_features.shape, edge_features.shape, from_idx.shape, to_idx.shape, graph_idx.shape, labels.shape)
        # print('node_features', node_features, '\n edge_features', edge_features, '\n from_idx', from_idx, '\n to_idx', to_idx, '\n graph_idx', graph_idx, '\n labels', labels)
        labels = labels.to(device)
    else:
        node_features, edge_features, from_idx, to_idx, graph_idx = get_graph(batch)
    #graph_vectors = model(node_features.to(device), edge_features.to(device), from_idx.to(device), to_idx.to(device), graph_idx.to(device), n_graphs.to(device))
        ...
        if ((i_iter + 1) // config['training']['print_after'] %
            ...
            with torch.no_grad():
                ...
                for batch in validation_set.triplets(config['evaluation']['batch_size']):
                    #n_graphs_double = torch.tensor([training_n_graphs_in_batch * 2])
                    node_features, edge_features, from_idx, to_idx, graph_idx = get_graph(batch)
                ...
                save_path = 'GMN_models'
                save_model(model, 1, save_path, (i_iter + 1))
